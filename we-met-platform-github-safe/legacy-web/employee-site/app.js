(() => {
  ...
  async function connect() {
    ...
    socket.on('call:connected', () => {
      $('#callState').textContent = 'Connected · member billing is active';
      P.toast('Member connected.', 'success');
    });
    socket.on('call:audio-paused', () => {
      if (currentCall) $('#callState').textContent = 'Audio paused · member is not being charged';
    });
    socket.on('call:audio-restored', () => {
      if (currentCall) $('#callState').textContent = 'Connected · member billing is active';
    });
  ...
  async function reportId(id, high) {
    const reason = prompt(high ? 'Explain why you want this member restricted:' : 'Describe the issue you want to report:');
    if (!reason) return;
    try {
