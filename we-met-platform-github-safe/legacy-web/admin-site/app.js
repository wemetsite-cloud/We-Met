(() => {
  ...
  const pageMeta = {
    ...
    customers: ['Members', 'Accounts, contact records and wallet controls'],
    ...
    support: ['Support', 'Member messages and replies'],
  ...
  async function loadLive(silent = false) {
    try {
      ...
      $('#mConcurrentBreakdown').textContent = `${Number(byRole.customer || 0)} members • ${Number(byRole.employee || 0)} listeners`;
  ...
  async function loadUsers() {
    try {
      ...
      renderCustomers();
      renderEmployees();
      $('#broadcastUser').innerHTML = '<option value="">All members and listeners</option>' + users.filter(user => user.role !== 'admin').map(user => `<option value="${user.id}">${P.esc(user.name)} — ${P.esc(user.email || user.username || '')}</option>`).join('');
    } catch (error) { P.toast(error.message, 'error'); }
  }
  ...
  async function reviewPayment(id, action) {
    ...
    const message = action === 'declined' ? (prompt('Reason shown to the member (optional):') || '') : '';
    try {
