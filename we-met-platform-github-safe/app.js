(() => {
  ...
  const IMAGE_LOADING_DELAY = 320;
  const SHOWCASE_CACHE_KEY = 'we_met_listener_showcase_v1';
  ...
  function cachedShowcaseImages() {
    try {
      const parsed = JSON.parse(localStorage.getItem(SHOWCASE_CACHE_KEY) || '{}');
      return Array.isArray(parsed.images) ? parsed.images.filter(Boolean) : [];
    } catch { return []; }
  }

  function cacheShowcaseImages(images) {
    try { localStorage.setItem(SHOWCASE_CACHE_KEY, JSON.stringify({ images, savedAt: Date.now() })); } catch {}
  }

  function prewarmImage(src, highPriority = false) {
  ...
  function renderListenerShowcase(images) {
    if (!images.length) { clearShowcaseLoading(); return; }
    const publicFrame = $('#listenerPublicShowcase');
    if (publicFrame) publicFrame.innerHTML = Array.from({length: Math.min(3, images.length)}, (_,i) => `<span><img src="${P.esc(images[i % images.length])}" alt="" loading="eager" decoding="async" fetchpriority="${i === 0 ? 'high' : 'auto'}"></span>`).join('');
    const home = $('#listenerHomeShowcase');
    if (home) {
      home.classList.remove('wm-image-loading-host');
      home.innerHTML = `<img src="${P.esc(images[0])}" alt="Listener community preview" loading="eager" decoding="async" fetchpriority="high"><span>${images.length > 1 ? `<img src="${P.esc(images[1])}" alt="" loading="eager" decoding="async" fetchpriority="high">` : ''}</span>`;
    }
  }

  async function loadListenerShowcase() {
    const cached = dailyShuffle(cachedShowcaseImages());
    if (cached.length) {
      renderListenerShowcase(cached);
      cached.slice(0, 3).forEach((src, index) => prewarmImage(src, index < 2));
    }
    try {
      const response = await P.api('/api/public/showcase-images', { cache: 'no-store' });
      const raw = Array.isArray(response.images) ? response.images.filter(Boolean) : [];
      if (!raw.length) { if (!cached.length) clearShowcaseLoading(); return; }
      cacheShowcaseImages(raw);
      const images = dailyShuffle(raw);
      renderListenerShowcase(images);
      images.slice(0, 2).forEach((src) => prewarmImage(src, true));
      images.slice(2, 5).forEach((src) => prewarmImage(src, false));
    } catch { if (!cached.length) clearShowcaseLoading(); }
  }
