...
const workflowSources = {
  ...
  customerWebrtc: fs.readFileSync(path.join(root, 'customer-site/webrtc.js'), 'utf8'),
  employeeWebrtc: fs.readFileSync(path.join(root, 'employee-site/webrtc.js'), 'utf8'),
...
const invariants = [
  ...
  [!workflowSources.socket.includes('pushService?.sendToUser') && !workflowSources.employeeWorker.includes("addEventListener('push'") && !workflowSources.employeeHtml.includes('enablePushBtn') && !workflowSources.employeeUi.includes('subscribeToPush') && workflowSources.push.includes("role === 'employee'"), 'Listener calling must not depend on Web Push or notification permission.'],
  [schema.includes('listener_availability') && workflowSources.socket.includes('hasLiveSocket') && workflowSources.socket.includes("employee.state === 'available' && hasLiveSocket(id)") && workflowSources.socket.includes("employees.delete(user.id)") && !workflowSources.socket.includes('hydratePersistentEmployees'), 'Only listeners with a live real-time socket may be shown as callable; disconnected listeners must not remain as ghost-online entries.'],
  [workflowSources.employeeUi.includes("sendAvailabilityCommand('employee:online')") && workflowSources.employeeUi.includes("sendAvailabilityCommand('employee:offline')") && workflowSources.socket.includes("listener_availability='online'") && workflowSources.socket.includes("listener_availability='offline'") && workflowSources.socket.includes('availabilityReply'), 'Listener Online/Break/Offline controls must be reversible and persisted without notification setup.'],
  [workflowSources.socket.includes("socket.on('webrtc:ready'") && workflowSources.socket.includes("'webrtc:start'") && workflowSources.customerUi.includes("socket.emit('webrtc:ready'") && workflowSources.employeeUi.includes("socket.emit('webrtc:ready'") && workflowSources.customerWebrtc.includes('async createOffer()') && workflowSources.employeeWebrtc.includes('async prepare(callId)'), 'Voice calls must use a two-sided WebRTC readiness handshake before the offer is created.'],
  [workflowSources.socket.includes('runtime.activating') && workflowSources.socket.includes('runtime.mediaConnected.size !== 2') && workflowSources.customerUi.includes("socket.emit('call:media-state'") && workflowSources.employeeUi.includes("socket.emit('call:media-state'"), 'A call may become active and billable only after both audio peers report a live media connection.'],
  ...
  [workflowSources.customerCss.includes('Themed scrolling and the main listener card slider') && workflowSources.customerCss.includes('*::-webkit-scrollbar-thumb') && workflowSources.customerCss.includes('#listenerGrid{') && workflowSources.customerCss.includes('grid-auto-flow:column') && !workflowSources.customerCss.includes('#listenerGrid,#favoritesList') && workflowSources.employeeCss.includes('Themed scrolling and fitted mobile cards') && workflowSources.employeeCss.includes('.stats{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))') && workflowSources.adminCss.includes('Themed scroll surfaces and fitted mobile summaries') && workflowSources.adminCss.includes('.employee-cards .actions{display:grid'), 'Only the main customer listener row may swipe horizontally; other customer, listener and admin cards must fit their mobile viewport.'],
