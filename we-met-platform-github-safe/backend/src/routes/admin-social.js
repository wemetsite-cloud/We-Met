  ...
  const [subscriptions, payments, topups, summary] = await Promise.all([
    db.query(`
      SELECT s.id,s.razorpay_subscription_id,s.status,s.current_period_start,
             s.current_period_end,s.cancel_at_cycle_end,s.paid_count,s.access_source,s.grant_note,s.revoked_at,s.created_at,s.updated_at,
    ...
    db.query(`
      SELECT
        COUNT(*) FILTER (WHERE
          ...
          OR (access_source='razorpay' AND status IN ('active','cancelled','completed') AND current_period_end>now())
