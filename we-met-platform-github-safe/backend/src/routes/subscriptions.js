...
async function recordSubscriptionPayment(client, subscription, payment) {
  ...
  if (!PAYMENT_PATTERN.test(paymentId) || !Number.isInteger(amountPaise) || amountPaise <= 0) return { credited: false };
  if (String(payment?.subscription_id || '') !== subscription.razorpay_subscription_id
      || String(payment?.currency || '') !== 'INR') return { credited: false };
  ...
  if (!record || record.listener_credited_at || record.status !== 'captured' || listenerCreditPaise <= 0) return { credited: false };
  await client.query(`
  ...
  return { credited: true, listenerCreditPaise };
}
...
async function updateSubscriptionFromProvider(client, record, providerSubscription, { eventAt = null } = {}) {
  ...
  const fallbackEnd = effectiveStatus === 'active' ? new Date(Date.now() + 32 * 24 * 60 * 60 * 1000) : null;
...
router.post('/create', authenticate, requireRole('customer'), createSubscriptionLimit, asyncHandler(async (req, res) => {
    ...
    const existing = await client.query(`
      ...
      WHERE customer_id=$1 AND employee_id=$2
        AND (
          ...
          OR (status IN ('created','authenticated','pending') AND updated_at>now()-interval '2 hours')
        )
  ...
  if (prepared.reused && prepared.record.status === 'active' && new Date(prepared.record.current_period_end) > new Date()) {
...
router.post('/verify', authenticate, requireRole('customer'), verifySubscriptionLimit, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    console.error('Razorpay subscription verification fetch failed:', error?.error?.code || error?.message || providerStatus(error));
    return res.status(502).json({ error: 'The membership payment status could not be confirmed. Please try again.' });
  }

  if (String(providerSubscription?.plan_id || '') !== config.razorpay.subscriptionPlanId) {
  ...
  if (String(providerPayment?.status || '') !== 'captured') {
    return res.status(409).json({ error: 'Razorpay has not captured this membership payment yet.' });
  }
  if (String(providerSubscription?.status || '') !== 'active') {
    return res.status(409).json({ error: 'Payment was received, but Razorpay is still activating the membership. Open Exclusive and refresh in a moment.' });
  }
  ...
  const outcome = await db.transaction(async (client) => {
    ...
    await client.query('UPDATE listener_subscriptions SET last_payment_id=$2,updated_at=now() WHERE id=$1', [updated.id, paymentId]);
    await client.query(`
...
async function webhook(req, res) {
  ...
  try {
    const notification = await db.transaction(async (client) => {
      ...
      if (providerSubscription && String(providerSubscription.plan_id || '') !== config.razorpay.subscriptionPlanId) return null;
      ...
      if (providerPayment?.id) {
        await client.query('UPDATE listener_subscriptions SET last_payment_id=$2,updated_at=now() WHERE id=$1', [local.id, providerPayment.id]);
      }
