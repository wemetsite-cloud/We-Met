...
router.get('/legal/:type', (req, res) => {
  const documents = {
    ...
    privacy: `We Met stores the information required to operate the service: account details, customer wallet balance, call duration and status, listener rates and earnings ledgers, listener payout details and payment references, text messages sent during an active call, Razorpay order and payment references, reports, notifications and support messages. We Met does not store card details, UPI credentials, UPI PINs or OTPs. Customer phone numbers and email addresses are not shown to listeners. Voice audio is not recorded by We Met. Service providers may process information only as required to host and operate the platform.`,
