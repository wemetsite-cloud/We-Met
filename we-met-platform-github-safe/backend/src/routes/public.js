...
router.get('/legal/:type', (req, res) => {
  const documents = {
    ...
    privacy: `We Met stores the information required to operate the service: account details, customer wallet balance, call duration and status, listener rates and earnings ledgers, listener payout details and payment references, text messages sent during an active call, direct-UPI customer payment references and verification screenshots, reports, notifications and support messages. Payment screenshots are accessible only to authorised administrators and may be removed after the required record-retention period. We Met does not ask for or store your UPI PIN or OTP. Customer phone numbers and email addresses are not shown to listeners. Voice audio is not recorded by We Met. Service providers may process information only as required to host and operate the platform.`,
