...
router.get('/legal/:type', (req, res) => {
  const documents = {
    ...
    privacy: `We Met stores account information, wallet balance, call duration and status, text messages sent inside an active call, payment references and status, required direct-UPI payment screenshots, reports, notifications and support messages so the service can operate. Payment screenshots may be removed after the required record-retention period. Razorpay processes gateway payment details under its own privacy and security terms when that gateway is enabled; We Met does not store your card number, UPI PIN or OTP. Customer phone numbers and email addresses are not shown to listeners. Voice audio is not recorded by this version of the platform. Database and service providers may process information only as required to operate the service.`,
    refund: `Talk-time is credited after an administrator approves the submitted direct-UPI UTR and screenshot, or after Razorpay confirms that a gateway payment is captured when Razorpay is enabled. Failed, cancelled, incomplete, declined or duplicate payment attempts do not receive talk-time. Purchased talk-time is not exchangeable for cash and is not refundable after use, except where required by law or expressly approved by the administrator. Contact support with the UPI transaction ID or Razorpay payment reference if a completed payment is not reflected in your wallet.`,
