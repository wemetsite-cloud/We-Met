...
router.get('/subscriptions', asyncHandler(async (req, res) => {
  ...
  const result = await db.query(`
    SELECT s.id,s.employee_id,s.status,s.current_period_start,s.current_period_end,
           s.cancel_at_cycle_end,s.paid_count,s.created_at,s.updated_at,
  ...
  res.json({ subscriptions: result.rows.map((row) => ({
    ...
    availability: row.listener_availability || 'offline',
    status: row.status,
    active: row.status === 'active' && new Date(row.current_period_end) > new Date(),
...
router.get('/conversations', asyncHandler(async (req, res) => {
  const result = await db.query(`
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE m.subscription_id=s.id ORDER BY created_at DESC LIMIT 1
...
router.get('/conversations/:employeeId/messages', asyncHandler(async (req, res) => {
  ...
  await db.query(`
    ...
    WHERE subscription_id=$1 AND sender_id=$2 AND read_at IS NULL
  `, [subscription.id, req.params.employeeId]);
