...
async function settleCallWithClient(callId, client) {
  ...
  // The listener row is the common serialization point for call credits,
  // administrator adjustments and exact-balance payout operations.
...
async function settleCall(callId, database = db) {
  if (!callId) return null;
  return database.transaction((client) => settleCallWithClient(callId, client));
}

module.exports = { settleCall, settleCallWithClient };
