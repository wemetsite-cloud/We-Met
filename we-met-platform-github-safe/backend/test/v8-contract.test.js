...
test('v8.3 customer home contains only connection, language choice and verified discovery', () => {
  ...
  assert.match(app, /checkout\.html\?plan=/);
...
test('v8.3 screenshot regressions keep calls, profiles, messages and checkout in the intended UI', () => {
  ...
  const walletCheckout = read('customer-site', 'checkout.js');
  const walletCheckoutHtml = read('customer-site', 'checkout.html');
  const walletCheckoutCss = read('customer-site', 'checkout.css');
  const listenerHtml = read('employee-site', 'index.html');
  ...
  assert.match(walletCheckout, /assets\/icon-192\.png/);
  assert.match(walletCheckout, /redirect: false/);
  assert.match(membershipCheckout, /assets\/icon-192\.png/);
  assert.match(membershipCheckout, /redirect: false/);
  assert.match(membershipCheckout, /backdrop_color: '#0c0d10'/);
  assert.match(walletCheckoutHtml, /id="checkoutView"/);
  assert.match(walletCheckoutHtml, /id="paymentChooser"/);
  assert.match(walletCheckout, /checkoutClient\.createPayment/);
  assert.match(walletCheckout, /\/v1\/razorpay\.js/);
  assert.doesNotMatch(walletCheckout, /\.open\(\)/);
  assert.match(walletCheckoutCss, /\.payment-method-tabs/);
  assert.doesNotMatch(walletCheckoutCss, /\.razorpay-container/);
...
test('v8.7 keeps listener media fresh and uses customer-controlled wallet checkout', () => {
  ...
  const checkoutCss = read('customer-site', 'checkout.css');
  const listenerApp = read('employee-site', 'app.js');
  const listenerCss = read('employee-site', 'style.css');
  ...
  assert.match(customerApp, /window\.location\.assign\(`checkout\.html\?plan=/);
  assert.doesNotMatch(customerApp, /walletCheckoutSection|wallet-checkout-active|activeWalletRazorpay/);
  assert.match(checkoutHtml, /Secure payment powered by Razorpay/);
  assert.match(checkoutApp, /redirect: false/);
  assert.match(checkoutApp, /checkoutClient\.createPayment/);
  assert.doesNotMatch(checkoutApp, /\.open\(\)/);
  assert.match(checkoutCss, /\.payment-method-tabs/);
  assert.doesNotMatch(checkoutCss, /\.razorpay-container/);
  assert.doesNotMatch(checkoutHtml, /admin|administrator|merchant/i);
  ...
  assert.match(customerHeaders, /display-capture=\(\)/);
  assert.match(checkoutHtml, /privacy-guard\.js\?v=8\.7\.0/);
  assert.match(customerApp, /service-worker\.js\?v=8\.7\.0/);
  assert.match(read('customer-site', 'privacy-guard.js'), /capture-obscured/);
  assert.match(read('customer-site', 'privacy-guard.css'), /@media print/);
