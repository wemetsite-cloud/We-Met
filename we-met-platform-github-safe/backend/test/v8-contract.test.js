...
test('v8.3 screenshot regressions keep calls, profiles, messages and checkout in the intended UI', () => {
  ...
  assert.match(walletCheckout, /checkout\.open\(\)/);
  assert.match(walletCheckoutCss, /\.razorpay-container \.razorpay-backdrop/);
...
test('v8.6 keeps listener media fresh, ships the supplied default banner and permits embedded Razorpay checkout', () => {
  ...
  assert.match(checkoutCss, /\.razorpay-container \.razorpay-backdrop/);
