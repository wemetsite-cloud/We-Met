...
test('ships authenticated Razorpay checkout without exposing the secret to the frontend', () => {
  ...
  assert.match(checkoutApp, /function loadRazorpayCheckout/);
  assert.match(checkoutApp, /script\.src = 'https:\/\/checkout\.razorpay\.com\/v1\/checkout\.js'/);
...
test('isolates portal sessions by role and clears stale mismatched tokens', () => {
  ...
  for (const [site, role, tokenKey] of portals) {
    ...
    assert.match(serviceWorker, new RegExp(`const VERSION = '${site === 'customer-site' ? '8\\.6\\.0' : '8\\.5\\.0'}'`));
