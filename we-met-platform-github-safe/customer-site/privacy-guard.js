(() => {
  ...
  function clearCaptureClipboard() {
    if (!navigator.clipboard?.writeText || !window.isSecureContext) return;
    navigator.clipboard.writeText('Screen capture is disabled on We Met.').catch(() => {});
  }
  ...
  document.addEventListener('freeze', obscure);
  ...
  document.addEventListener('keydown', (event) => {
    ...
    obscure();
    if (key === 'printscreen') {
      clearCaptureClipboard();
      window.setTimeout(clearCaptureClipboard, 180);
    }
