(() => {
  ...
  async function registerServiceWorker() {
    ...
    try { await navigator.serviceWorker.register('service-worker.js?v=8.9.2', { updateViaCache: 'none' }); } catch {}
  }
  ...
  async function handleActionClick(event) {
    ...
    const target = event.target.closest('button[data-listener-profile],button[data-follow],button[data-subscribe],button[data-listener-call],button[data-listener-message],button[data-conversation],button[data-cancel-subscription]');
    if (!target) return;
  ...
  async function openListenerProfile(listenerId) {
    ...
    try {
      ...
      // Direct subscription launch keeps the Razorpay call attached to the real button click.
      $$('#listenerProfileContent [data-subscribe]').forEach((button) => {
        button.onclick = (event) => {
          event.preventDefault();
          event.stopPropagation();
          subscribeToListener(button.dataset.subscribe, event.currentTarget);
        };
      });
  ...
  async function loadPlans() {
    if (!me) return;
    try {
      const response = await P.api('/api/customer/plans', { cache: 'no-store' });
      paymentPlans = response.plans || [];
      $('#walletPaymentIntro').textContent = 'Choose a talk-time pack.';
      $('#plansGrid').innerHTML = paymentPlans.length
        ...
        : emptyState('No talk-time packs', 'Talk-time packs will appear here.');

      // Keep payment launch on the button itself, like the known-good light-theme build.
      $$('[data-buy-plan]').forEach((button) => {
        button.onclick = (event) => {
          event.preventDefault();
          event.stopPropagation();
