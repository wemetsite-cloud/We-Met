(() => {
  ...
  async function registerServiceWorker() {
    ...
    try {
      serviceWorkerRegistration = await navigator.serviceWorker.register('service-worker.js?v=6.9.2', { updateViaCache: 'none' });
  ...
  async function connectSocket() {
    ...
    socket.on('connect', () => {
      ...
      if (currentCall) {
        $('#callState').textContent = 'Restoring the secure call…';
        if (['connecting', 'active'].includes(currentCall.status)) {
          socket.emit('call:media-state', {
            callId: currentCall.id,
            connected: audioCall?.isConnected() === true,
          });
        }
      }
    });
    socket.on('disconnect', () => {
      if (!currentCall) return;
      currentCall.mediaConnected = false;
      $('#callState').textContent = 'Reconnecting… talk-time is paused';
      P.toast('Connection interrupted. Restoring your call…', 'info');
    });
    socket.on('connect_error', (error) => console.warn('Calling server reconnecting:', error?.message || error));
    socket.on('call:idle', () => {
      if (!currentCall) return;
      closeCall();
      P.toast('The previous call is no longer active.', 'info');
    });
    socket.on('call:resume', async (data) => {
      const hadCall = currentCall?.id === data.callId;
      const sameMediaCall = audioCall?.callId === data.callId;
      currentCall = {
        ...currentCall,
        id: data.callId,
        employee: data.employee || currentCall?.employee,
        status: data.status,
        billed: Number(data.billedSeconds || 0),
        mediaConnected: false,
      };
      if (!hadCall) openCall();
      $('#callTimer').textContent = P.duration(currentCall.billed);
      $('#restoreTimer').textContent = P.duration(currentCall.billed);
      if (Number.isFinite(Number(data.balanceSeconds))) updateBalance(Number(data.balanceSeconds));

      if (data.status === 'connecting') {
        $('#callState').textContent = 'Restoring secure audio…';
        try {
          if (!sameMediaCall) await audioCall.prepare(data.callId);
          socket.emit('webrtc:ready', { callId: data.callId });
        } catch (error) {
          P.toast(error.message || 'The microphone could not be restored.', 'error');
          socket.emit('call:end', { callId: data.callId });
        }
        return;
      }

      const mediaConnected = sameMediaCall && audioCall.isConnected();
      currentCall.mediaConnected = mediaConnected;
      $('#callState').textContent = mediaConnected
        ? 'Connected · billing is active'
        : 'Restoring audio… talk-time is paused';
      socket.emit('call:media-state', { callId: data.callId, connected: mediaConnected });
    });
    ...
    socket.on('session:revoked', (data) => {
      P.toast(data.reason || 'Your login changed. Sign in again.', 'error');
      P.Store.clear();
      logout(false);
    });
