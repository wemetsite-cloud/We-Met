(() => {
  ...
  async function handleActionClick(event) {
    ...
    if (d.resetUserPassword) return resetUserPasswordModal(d.resetUserPassword);
    if (d.grantExclusive) return grantExclusiveModal(d.grantExclusive);
    if (d.revokeExclusive) return revokeExclusiveAccess(d.exclusiveCustomer, d.revokeExclusive);
  ...
  async function showDetails(id) {
    try {
      ...
      const exclusiveRows = (data.exclusiveAccesses || []).map((entry) => {
        const counterpart = user.role === 'customer'
          ? (entry.listener_username ? `@${entry.listener_username}` : entry.listener_private_name)
          : entry.customer_name;
        const source = entry.access_source === 'admin' ? 'Admin access' : '₹399 autopay';
        const period = entry.access_source === 'admin'
          ? (entry.access_active ? 'Until administrator removes access' : `Removed ${P.date(entry.revoked_at || entry.updated_at)}`)
          : (entry.current_period_end ? `Paid access until ${P.date(entry.current_period_end)}` : 'Waiting for first paid period');
        const renewal = entry.access_source === 'razorpay'
          ? (entry.cancel_at_cycle_end ? 'Renewal off' : 'Auto-renewal on')
          : `Granted by ${entry.granted_by_name || 'administrator'}`;
        const revoke = user.role === 'customer' && entry.access_source === 'admin' && entry.access_active
          ? `<button class="danger" type="button" data-revoke-exclusive="${P.esc(entry.id)}" data-exclusive-customer="${P.esc(entry.customer_id)}">Remove access</button>`
          : '';
        return `<div class="profile-row"><div><strong>${P.esc(counterpart || 'Unknown account')}</strong><small>${P.esc(source)} • ${P.esc(renewal)}</small></div><span class="pill ${entry.access_active ? 'active' : 'expired'}">${entry.access_active ? 'accessible' : 'locked'}</span><small>${P.esc(period)}</small>${revoke}</div>`;
      }).join('') || '<p>No exclusive access or autopay history.</p>';
      const exclusiveControls = user.role === 'customer'
        ? `<div class="customer-card-actions"><button class="primary" type="button" data-grant-exclusive="${P.esc(user.id)}">Unlock with listener</button></div>`
        : '';
      const accountControls = user.role !== 'admin'
        ? `<section class="profile-section"><h3>Account controls</h3><div class="customer-card-actions"><button class="warning" type="button" data-reset-user-password="${P.esc(user.id)}">Reset password</button></div></section>`
        : '';
  ...
  function resetUserPasswordModal(id) {
    const user = users.find((item) => item.id === id && item.role !== 'admin');
    if (!user) return;
    modal(`Reset password — ${user.name}`, `<form id="resetUserPasswordForm" class="stack"><p class="modal-copy">Set a temporary password and share it securely with this ${P.esc(user.role)}. All current sessions will be signed out.</p><label>New temporary password<input id="resetUserPasswordValue" type="password" minlength="8" maxlength="128" autocomplete="new-password" required></label><label>Confirm password<input id="resetUserPasswordConfirm" type="password" minlength="8" maxlength="128" autocomplete="new-password" required></label><button class="warning">Reset password</button></form>`);
    $('#resetUserPasswordForm').onsubmit = async (event) => {
      event.preventDefault();
      const password = $('#resetUserPasswordValue').value;
      if (password !== $('#resetUserPasswordConfirm').value) return P.toast('The two passwords do not match.', 'error');
      try {
        const result = await P.api(`/api/admin/users/${encodeURIComponent(id)}/reset-password`, { method: 'POST', body: JSON.stringify({ newPassword: password }) });
        closeAdminModal();
        P.toast(result.message || 'Password reset.', 'success');
        loadAudit();
      } catch (error) { P.toast(error.message, 'error'); }
    };
  }

  function grantExclusiveModal(customerId) {
    const customer = users.find((item) => item.id === customerId && item.role === 'customer');
    const listeners = users.filter((item) => item.role === 'employee' && item.status === 'active');
    if (!customer) return;
    if (!listeners.length) return P.toast('Create or activate a listener first.', 'info');
    modal(`Give exclusive access — ${customer.name}`, `<form id="grantExclusiveForm" class="stack"><p class="modal-copy">This unlocks one listener’s exclusive posts and messages without starting an autopay mandate. You can remove it from the customer profile later.</p><label>Listener<select id="grantExclusiveListener" required><option value="">Choose listener</option>${listeners.map((listener) => `<option value="${P.esc(listener.id)}">${P.esc(listener.username ? `@${listener.username}` : listener.name)}</option>`).join('')}</select></label><label>Admin note<input id="grantExclusiveNote" maxlength="500" value="Exclusive access granted by administrator"></label><button class="primary">Unlock exclusive access</button></form>`);
    $('#grantExclusiveForm').onsubmit = async (event) => {
      event.preventDefault();
      try {
        await P.api(`/api/admin/users/${encodeURIComponent(customerId)}/exclusive-access`, { method: 'POST', body: JSON.stringify({ employeeId: $('#grantExclusiveListener').value, note: $('#grantExclusiveNote').value }) });
        P.toast('Exclusive access unlocked.', 'success');
        await showDetails(customerId);
        loadPayments(); loadAudit();
      } catch (error) { P.toast(error.message, 'error'); }
    };
  }

  async function revokeExclusiveAccess(customerId, subscriptionId) {
    if (!customerId || !subscriptionId || !confirm('Remove this administrator-granted exclusive access now?')) return;
    try {
      await P.api(`/api/admin/users/${encodeURIComponent(customerId)}/exclusive-access/${encodeURIComponent(subscriptionId)}`, { method: 'DELETE' });
      P.toast('Exclusive access removed.', 'success');
      await showDetails(customerId);
      loadPayments(); loadAudit();
    } catch (error) { P.toast(error.message, 'error'); }
  }
  ...
  async function loadPayments() {
    try {
      ...
      $('#adminSubscriptions').innerHTML = adminPayments.subscriptions?.length ? adminPayments.subscriptions.slice(0,300).map(item => `<article class="admin-payment-row"><div><span class="pill ${P.esc(item.status)}">${P.esc(item.status)}</span><strong>${P.esc(item.customer_name)} → @${P.esc(item.listener_name)}</strong><small>${item.access_source === 'admin' ? 'Administrator-granted access' : P.esc(item.razorpay_subscription_id)}</small></div><div><b>${item.access_source === 'admin' && !item.revoked_at ? 'Direct access' : item.current_period_end ? `Until ${P.date(item.current_period_end)}` : 'Not active'}</b><small>${item.access_source === 'admin' ? P.esc(item.grant_note || 'Granted by administrator') : item.cancel_at_cycle_end ? 'Renewal off' : `${Number(item.paid_count || 0)} paid cycle(s)`}</small></div></article>`).join('') : '<p class="empty-copy">No listener memberships yet.</p>';
