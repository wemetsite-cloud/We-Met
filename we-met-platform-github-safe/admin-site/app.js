(() => {
    ...
    async function showDetails(id) { try {
        ...
        modal(`Details — ${u.name}`, `<div class="stack"><div class="mini-card"><div><strong>${P.esc(u.name)}</strong><p>${P.esc(u.email || u.username || '')} • ${P.esc(u.role)}</p><p>Balance: ${P.duration(u.balance_seconds)} • Status: ${P.esc(u.status)}</p></div></div><h3>Recent calls</h3>${d.calls.length ? d.calls.slice(0, 10).map(c => `<div class="mini-card"><div><strong>${P.esc(c.customer_name)} ↔ ${P.esc(c.employee_name)}</strong><p>${P.duration(c.billed_seconds)} • ${P.esc(c.status)} • ${P.date(c.created_at)}</p></div></div>`).join('') : '<p>No calls.</p>'}<h3>Wallet</h3>${d.wallet.length ? d.wallet.slice(0, 10).map(w => `<div class="mini-card"><div><strong>${w.seconds_delta > 0 ? '+' : '−'}${P.duration(Math.abs(w.seconds_delta))}</strong><p>${P.esc(w.note || w.type)} • ${P.date(w.created_at)}</p></div></div>`).join('') : '<p>No wallet entries.</p>'}<h3>Reports / support</h3><p>${d.reports.length} report(s), ${d.support.length} support ticket(s)</p></div>`);
    }
    catch (e) {
        P.toast(e.message, 'error');
    } }
