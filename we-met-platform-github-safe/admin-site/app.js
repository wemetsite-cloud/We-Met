(() => {
  ...
  async function loadPayments() {
    try {
      ...
      $('#paymentsList').innerHTML = payments.map(payment => `<article class="ticket payment-review"><div><span class="pill ${P.esc(payment.status)}">${P.esc(payment.status)}</span><strong>${profileLink(payment.customer_id, payment.customer_name, payment.customer_email)} — ${P.esc(payment.plan_name)}</strong><p>${P.money(payment.amount_paise)} • ${Math.round(payment.seconds / 60)} minutes • Direct UPI • ${P.date(payment.created_at)}</p><p>${payment.customer_phone ? `<a class="phone-link" href="tel:${P.esc(payment.customer_phone)}">${P.esc(payment.customer_phone)}</a>` : 'Phone not provided'}</p><div class="payment-review-refs"><p class="payment-utr"><b>UPI transaction ID / UTR</b><strong>${P.esc(payment.utr_reference || 'Missing')}</strong>${payment.utr_reference ? `<button class="ghost compact-copy" data-copy="${P.esc(payment.utr_reference)}">Copy</button>` : ''}</p><p><b>Paid to</b><strong>${P.esc(payment.payee_upi_id || 'Unknown')}</strong></p><p><b>Checkout reference</b><strong>${P.esc(payment.checkout_reference || '—')}</strong></p></div>${payment.customer_note ? `<p><b>Customer note:</b> ${P.esc(payment.customer_note)}</p>` : ''}${payment.admin_message ? `<p><b>Admin message:</b> ${P.esc(payment.admin_message)}</p>` : ''}</div><div class="actions payment-review-actions">${payment.proof_size ? `<button class="ghost" data-payment-proof="${payment.id}">View screenshot</button>` : '<span class="payment-note-label">Screenshot missing</span>'}${payment.status === 'pending' ? `<button class="primary" data-payment-approve="${payment.id}" ${!payment.proof_size || !payment.utr_reference ? 'disabled' : ''}>Approve</button><button class="danger" data-payment-decline="${payment.id}">Decline</button>` : ''}</div></article>`).join('') || '<p>No UPI submissions yet.</p>';
    } catch (error) { P.toast(error.message, 'error'); }
  }
  ...
  async function reviewPayment(id, action) {
    const payment = payments.find(item => item.id === id);
    if (!payment) return;
    if (action === 'approved' && !confirm(`Approve ${P.money(payment.amount_paise)} / ${Math.round(payment.seconds / 60)} minutes for ${payment.customer_name}?\n\nVerify first:\nUPI: ${payment.payee_upi_id || 'Unknown'}\nUTR: ${payment.utr_reference || 'Missing'}\nCheckout: ${payment.checkout_reference || '—'}`)) return;
