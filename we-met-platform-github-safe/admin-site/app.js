(() => {
    ...
    async function registerFreshServiceWorker() {
        if (!('serviceWorker' in navigator))
            return;
        try {
            const registration = await navigator.serviceWorker.register('service-worker.js?v=5.11.0', { updateViaCache: 'none' });
    ...
    function openPage(name, { historyMode = 'push' } = {}) { if (!pageMeta[name]) name = 'overview'; if (activePage !== name && historyMode === 'push') history.pushState({ marker: NAVIGATION_MARKER, page: name }, document.title); else if (historyMode === 'replace') history.replaceState({ marker: NAVIGATION_MARKER, page: name }, document.title); activePage = name; $('#adminBackButton').classList.toggle('hidden', name === 'overview'); $$('#nav button').forEach(b => b.classList.toggle('active', b.dataset.page === name)); $$('.page').forEach(p => p.classList.toggle('active', p.id === `page-${name}`)); $('#pageTitle').textContent = pageMeta[name][0]; $('#pageDesc').textContent = pageMeta[name][1]; window.scrollTo({ top: 0, behavior: 'smooth' }); if (name === 'overview') {
        loadDashboard();
        loadLive();
    ...
    function renderCustomers() { const q = ($('#customerSearch')?.value || '').toLowerCase(), list = users.filter(x => x.role === 'customer' && (`${x.name} ${x.email || ''} ${x.phone || ''}`).toLowerCase().includes(q)); $('#customersTable').innerHTML = list.map(u => `<tr><td><b>${P.esc(u.name)}</b></td><td>${P.esc(u.email || '—')}</td><td>${u.phone ? `<a class="phone-link" href="tel:${P.esc(u.phone)}">${P.esc(u.phone)}</a>` : '<span class="contact-missing">Not provided</span>'}</td><td>${P.duration(u.balance_seconds)}</td><td><span class="pill ${u.status}">${u.status}</span>${u.suspended_until ? `<br><small>until ${P.date(u.suspended_until)}</small>` : ''}</td><td>${P.date(u.created_at)}</td><td><div class="actions"><button class="ghost" data-details="${u.id}">Details</button><button class="ghost" data-minutes="${u.id}">Minutes</button><button class="warning" data-suspend="${u.id}">Suspend</button><button class="${u.status === 'blocked' ? 'ghost' : 'danger'}" data-block="${u.id}" data-status="${u.status}">${u.status === 'blocked' ? 'Activate' : 'Block'}</button><button class="ghost" data-reset="${u.id}">Reset pass</button></div></td></tr>`).join(''); wireUserActions(); }
    ...
    async function loadPayments() { try {
        ...
        const manual = payments.map(p => { const method = p.payment_method === 'bank_transfer' ? 'Older bank transfer' : 'Direct UPI'; return `<article class="ticket payment-review"><div><span class="pill ${p.status}">${P.esc(p.status)}</span><strong>${P.esc(p.customer_name)} — ${P.esc(p.plan_name)}</strong><p>${P.money(p.amount_paise)} • ${Math.round(p.seconds / 60)} minutes • ${P.esc(method)} • ${P.date(p.created_at)}</p><p>${P.esc(p.customer_email || '')}${p.customer_phone ? ` • <a class="phone-link" href="tel:${P.esc(p.customer_phone)}">${P.esc(p.customer_phone)}</a>` : ''}</p><p><b>UPI transaction ID / UTR:</b> ${P.esc(p.utr_reference || 'Missing')}${p.checkout_reference ? ` • Checkout ${P.esc(p.checkout_reference)}` : ''}${p.payee_upi_id ? ` • Receiver ${P.esc(p.payee_upi_id)}` : ''}</p>${p.customer_note ? `<p><b>Customer note:</b> ${P.esc(p.customer_note)}</p>` : ''}${p.admin_message ? `<p><b>Admin message:</b> ${P.esc(p.admin_message)}</p>` : ''}</div><div class="actions">${p.proof_size ? `<button class="ghost" data-payment-proof="${p.id}">View optional screenshot</button>` : '<span class="gateway-label">No screenshot attached</span>'}${p.status === 'pending' ? `<button class="primary" data-payment-approve="${p.id}">Match UPI record + approve</button><button class="danger" data-payment-decline="${p.id}">Decline</button>` : ''}</div></article>`; }).join('');
    ...
    async function viewPaymentProof(id) { try {
        ...
        modal(`Optional screenshot — ${payment?.customer_name || ''}`, `<img id="paymentProofImage" class="proof-image" alt="Uploaded payment screenshot"><p>This image is not proof of settlement. Match the exact amount and UPI transaction ID in the receiving UPI app or bank statement before approval.</p>`);
    ...
    async function reviewPayment(id, action) { const payment = payments.find(p => p.id === id); if (!payment) return; if (action === 'approved') { const check = String(payment.utr_reference || '').slice(-4).toUpperCase(); const typed = prompt(`Open the receiving UPI APP or BANK STATEMENT and match ${P.money(payment.amount_paise)} with transaction ID ${payment.utr_reference || 'missing'}. If it matches, type the last 4 characters: ${check}`); if (!typed || typed.trim().toUpperCase() !== check) return P.toast('Approval cancelled. The UPI transaction was not confirmed.', 'error'); }
        const message = prompt(`Optional message for the customer (${action}):`) || ''; try {
        await P.api(`/api/admin/payments/${id}`, { method: 'PATCH', body: JSON.stringify({ action, adminMessage: message, settlementRecordMatched: action === 'approved' }) });
