...
test('the customer portal ships a payment-free v8.8 layout and phone-first authentication', () => {
  const siteRoot = path.join(projectRoot, 'customer-site');
  ...
  const privacyGuard = fs.readFileSync(path.join(siteRoot, 'privacy-guard.js'), 'utf8');
  ...
  assert.doesNotMatch(html, /data-buy-plan|data-subscribe|Continue to payment/i);
  assert.doesNotMatch(app, /data-buy-plan|data-subscribe|checkout\.html|create-order|verify-payment/i);
  assert.doesNotMatch(css, /wallet-plan-card|wallet-checkout|payment-modal/i);
  assert.doesNotMatch(app, /data-video|Start video call/);
  assert.match(privacyGuard, /visibilitychange/);
  assert.match(privacyGuard, /printscreen/i);
